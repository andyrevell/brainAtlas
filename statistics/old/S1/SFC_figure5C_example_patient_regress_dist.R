...
for (z in 4){
  ...
  if (z == 9){sub_ID='RID0320'; iEEG_filename = 'HUP140_phaseII_D02';  ictal_start_time = 331979921071}
  ...
  fname = paste0(output_directory, '/Figure5C_sub-', sub_ID, '_', iEEG_filename,  '_ICTAL_', ictal_start_time,"2.pdf")
  pdf(fname , width = 14, height = 6)
...
filler =  rep(NA,  length(all_data_random_atlases$RA_N0010$ictal$mean$broadband) *0.20  )# X% of ictal length 
...
for (a in 1:length(standard_atlases_to_plot)){
  ...
  plot(x, y_moving_avg, xlim = xlim, ylim = ylim, bty="n", type = "l", lwd = standard_atlases_line_width, 
       col = standard_atlas_colors[a], ylab = "", xlab = "", xaxt = 'n', las = las, lty=line_type[a])
  par(xpd = F)
...
legend("right", inset=c(-0.12,0), legend = standard_atlases_legend[c(1,5,2,3,4)], bty = 'n', bg ='n',horiz = F,
       xpd = NA, x.intersp=0.3,
       xjust=0, yjust=0, cex = 1.1, lty = line_type[c(1,5,2,3,4)], col =  standard_atlas_colors[c(1,5,2,3,4)], lwd = standard_atlases_line_width*0.6)
...
text(x = par("usr")[1],
     y = par("usr")[4]*0.9, labels = "  Standard Atlases",adj = c(0,0.5), cex = cex_main, font = 1)
