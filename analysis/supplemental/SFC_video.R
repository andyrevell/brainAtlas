...
figure_directory = "/Users/andyrevell/Desktop/supplemental_videos/SFC_video"
...
#for (t in 1:1){
  ...
  png(fname, width = 700, height = 900, units = "px", res = 300)
  par("mar" = c(1.2,1.5,0.5,0.2)) 
  par(mfrow=c(2,1))
  ...
  mgp.axis(1, cex.axis = cex, at = seq(0,1,0.1), mgp = c(0,-0.5,0))
  mgp.axis(2, cex.axis = cex, las = 1, mgp = c(0,0.55,0))
  title( xlab="Structural Weight (log normalized )", line = 0.3, cex.lab = cex)
  ...
  plot(seq( 0 - dim(data_preictal_broadband)[3],  length(data_broadband[1,1,] ) - dim(data_preictal_broadband)[3]-1,1),
    ...
    ylab = "", xlab = "", pch =19)
  mgp.axis(1, cex.axis = cex , mgp = c(0,-0.5,0))
  mgp.axis(2, cex.axis = cex , las = 1, mgp = c(0,0.55,0))
  title( xlab="Time (s)", line = 0.3, cex.lab = cex)
  ...
  plot(seq( 0 - dim(data_preictal_broadband)[3],  length(data_broadband[1,1,] ) - dim(data_preictal_broadband)[3]-1,1),
       ...
       ylim = ylim, bty="n", col = "black", las = 1, xaxt='n', yaxt = 'n',
       ylab = "", xlab = "", type="l", lwd=1.5)
  par(new=FALSE)
  dev.off()
