...
inputfile_MNI_template = ospj( path, "data_raw/MNI_brain_template/MNI152_T1_1mm_brain.nii.gz")
inputpath_electrode_localization = ospj( path, "data_raw/electrode_localization")
inputpath_atlases_standard = ospj( path, "data_raw/atlases/standard_atlases")
inputpath_atlases_random = ospj( path, "data_raw/atlases/random_atlases")
outputpath_electrode_localization = ospj( path, "data_processed/electrode_localization_atlas_region")
...
for i in range(len(sub_IDs_unique)):
    ...
    inputpath_electrode_localization_sub_ID = ospj(inputpath_electrode_localization, "sub-{0}".format(sub_ID))
    ...
    inputfile_electrode_localization_sub_ID_fullpath = ospj(inputpath_electrode_localization_sub_ID, inputfile_electrode_localization_sub_ID)
    #getting atlas names
    ...
    outputpath_electrode_localization_sub_ID = ospj(outputpath_electrode_localization, "sub-{0}".format(sub_ID))
    ...
    for a in range(len(atlas_names_standard)):
        inputfile_atlases_standard = ospj(inputpath_atlases_standard, atlas_names_standard[a] )
        ...
        outputfile_electrode_localization_fullpath = ospj(outputpath_electrode_localization_sub_ID, outputfile_electrode_localization)
        print("Subject: {0}  Atlas: {1}".format(sub_ID, atlas_names_standard[a]))
        electrode_localization.by_atlas(inputfile_electrode_localization_sub_ID_fullpath, inputfile_atlases_standard, inputfile_MNI_template, outputfile_electrode_localization_fullpath)
    ...
    for a in range(len(atlas_names_random)):
         for p in range(1, permutations+1):
            inputfile_atlases_random = ospj(inputpath_atlases_random, atlas_names_random[a], "{0}_v{1}.nii.gz".format(atlas_names_random[a], '{:04}'.format(p))  )
            ...
            electrode_localization.by_atlas(inputfile_electrode_localization_sub_ID_fullpath, inputfile_atlases_random, inputfile_MNI_template, outputfile_electrode_localization_fullpath)
