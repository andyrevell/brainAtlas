...
paper_processed_directory="/gdrive/public/USERS/arevell/papers/paper001/data_processed/structure_function_correlation_AS/regress_out_struc_distance"
#Random Atlases
...
for a in range(len(atlas_folder)):
    outputfile_directory = "{0}/sub-{1}/{2}".format(paper_processed_directory,sub_ID,atlas_folder[a])
    ...
    for p in range(len(perm)):
        ...
        for i in range(len(start_times_array)):
            ...
            structure_file_path =  "{0}/sub-{1}/connectivity_matrices/structural_regress_distance/{2}/{3}".format(BIDS_proccessed_directory,sub_ID,atlas_folder[a],structure_file_name)
            ...
            outputfile_name = "sub-{0}_{1}_{2}_{3}_{4}_Perm{5}_regress_distance_correlation.pickle".format(sub_ID,iEEG_filename,start_time_usec,stop_time_usec, atlas_folder[a], '{:04}'.format(perm[p]))
                ...
                if (os.path.exists(function_file_path)) == False:
                    ...
                    if (os.path.exists(electrode_localization_by_atlas_file_path))==False:
                        ...
                        if (os.path.exists(outputfile_directory))==False:
                            ...
                            get_structure_function_correlation.SFC_regress_out_distance(structure_file_path,function_file_path,electrode_localization_by_atlas_file_path, outputfile)
                        else:
            ...
            if (os.path.exists(function_file_path)) == False:
                ...
                if (os.path.exists(electrode_localization_by_atlas_file_path))==False:
                    ...
                    if (os.path.exists(outputfile_directory))==False:
                        print('{0} does not exist'.format(outputfile_directory))
                        os.mkdir(outputfile_directory)
                        get_structure_function_correlation.SFC_regress_struc_dist(structure_file_path,function_file_path,electrode_localization_by_atlas_file_path, outputfile)
