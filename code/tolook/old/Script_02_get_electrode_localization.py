...
def batch_compute_dist_from_grey(subjects):
    BIDS_dir = '/gdrive/public/DATA/Human_Data/BIDS_processed'
    mni_template_path = '/gdrive/public/TOOLS/atlases_and_templates/templates/MNI152_T1_1mm_brain.nii.gz'
    tissue_segmentation_path ='/gdrive/public/TOOLS/atlases_and_templates/atlases/tissue_res-1x1x1.nii.gz'
    for sub in subjects:
        electrode_coordinates_mni_path = '{0}/sub-{1}/electrode_localization/sub-{1}_electrode_coordinates_mni.csv'.format(BIDS_dir,sub)
        output_dir =  '{0}/sub-{1}/electrode_localization/electrode_localization_by_atlas'.format(BIDS_dir,sub)
        if (os.path.exists(output_dir)):
            file_name = 'sub-{0}_electrode_coordinates_mni_tissue_segmentation_dist_from_grey.csv'.format(sub)
            outputfile = '{0}/{1}'.format(output_dir,file_name)
            print('Saving: {0}'.format(file_name))
            get_electrode_localization.distance_from_grayMatter(electrode_coordinates_mni_path, mni_template_path, tissue_segmentation_path, outputfile)
        else:
            print('{0} directory does not exist'.format(output_dir))
            print('making directory')
            os.mkdir(output_dir)
...
def batch_localize_electrodes_one_atlas(subjects,standard_atlas):
    temp = standard_atlas
    atlas_dir_temp = '/gdrive/public/TOOLS/atlases_and_templates/atlases'
    BIDS_dir = '/gdrive/public/DATA/Human_Data/BIDS_processed'
    mni_template_path = '/gdrive/public/TOOLS/atlases_and_templates/templates/MNI152_T1_1mm_brain.nii.gz'
